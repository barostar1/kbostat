...
df = pd.read_csv('./data/2020pitcherstat.csv', index_col=0)  # 타자 데이터 로드 index_col = 0 <- unnamed index 제거
...
for i in tqdm(range(len(df)), mininterval=1): #진행상황 표시
    ...
    plt.xticks(larger_angles[:-1], larger_categories,
               color='grey', size=12)
    plt.yticks([0, 10, 30, 50, 70, 90], ['0', '10', '30', '50', '70', '90'],
               color='grey', size=12)
    plt.ylim(0, 90)
    ...
    # plt.show()
    plt.savefig('./static/pitcher_stat_img/' + df.loc[i][1] + '_' + df.loc[i][0] + '.png')
